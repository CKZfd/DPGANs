...
parser.add_argument('--testBatchSize', type=int, default=100, help='input batch size')
...
if __name__ == '__main__':
    ...
    for idx in range(val_input.size(0)):
        ...
        vutils.save_image(output[idx * 3 + 0, :, :, :], 'quality_img/%04dinput.png' % idx, normalize=True)
        vutils.save_image(output[idx * 3 + 1, :, :, :], 'quality_img/%04dreal.png' % idx, normalize=True)
        vutils.save_image(output[idx * 3 + 2, :, :, :], 'quality_img/%04dgenerate.png' % idx, normalize=True)
        input_t = cv2.imread('quality_img/%04dinput.png' % idx)
        real_t = cv2.imread('quality_img/%04dreal.png' % idx)
        generate_t = cv2.imread('quality_img/%04dgenerate.png' % idx)

        input_psnr = psnr1(input_t, real_t)
        generate_psnr = psnr1(generate_t, real_t)
        print("%d:" % idx, input_psnr)
        print("%d:" % idx, generate_psnr)
        ...
        print("%d:" % idx, input_ssim)
        print("%d:" % idx, generate_ssim)
        total_input_psnr += input_psnr
        total_generate_psnr += generate_psnr
        total_input_ssim += input_ssim
        total_generate_ssim += generate_ssim

        if generate_psnr > input_psnr+2.5  and generate_ssim > input_ssim+0.05:
            max_key.append(idx)
            max_value.append((input_psnr, generate_psnr, input_ssim, generate_ssim))

    vutils.save_image(output, '%s/15generated.png' % opt.exp, nrow=3, normalize=True)
    print("total_input_psnr :", total_input_psnr)
    print("total_generate_psnr :", total_generate_psnr)
    print("total_input_ssim :", total_input_ssim)
    print("total_generate_ssim :", total_generate_ssim)

    print(max_key)
    for i in range(0,len(max_value)):
        print(max_key[i], max_value[i])
